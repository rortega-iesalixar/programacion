...
public class Minecraft {
  ...
  public void borrarMaterialSinMasa() {
    for (int i = 0; i < this.indice_materiales; i++) {
      if (this.materiales[i].getMasa() <= 0) {
        for (int j = i; j < this.indice_materiales - 1; j++) {
          this.materiales[j] = this.materiales[j + 1];
        }
        i -= 1;
        this.indice_materiales -= 1;
        // this.materiales = Arrays.copyOf(this.materiales, this.materiales.length - 1);
      }
    }
  }
  ...
  public String mostrarEstado() {
    String cadena = "";
    for (int i = 0; i < this.indice_materiales; i++) {
      cadena = cadena + this.materiales[i].toString() + "\n";
    }
    return cadena;
  ...
  public void setMateriales() {
    this.materiales = new Material[this.getMax_materiales()];
  }
